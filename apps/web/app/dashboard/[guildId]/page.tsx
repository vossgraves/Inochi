...
const nav = [
  ...
  { href: "#commands", label: "Commands" }, { href: "#games", label: "Games" }, { href: "#roles", label: "Roles & community" }, { href: "#logging", label: "Logs & automation" }, { href: "#imports", label: "Data & backups" },
];
...
export default async function GuildDashboard({ params }: { params: Promise<{ guildId: string }> }) {
  ...
  return <DashboardShell guildName={access.guild.name} nav={nav}>
    <div className="page-heading" data-reveal><div><div className="eyebrow mono">Server control / {guildId}</div><h1>{access.guild.name}</h1><p>One configuration shared by the dashboard, Discord worker, and public API.</p></div><Link className="button" href={`/leaderboard/${guildId}`} target="_blank">Leaderboard <ExternalLink size={14} /></Link></div>
    <section className="overview-panel" id="overview" data-reveal>
      <div className={`overview-primary system-${row.settings.enabled ? "active" : "paused"}`}><div className="system-state-mark"><BrandMark state={row.settings.enabled ? "active" : "paused"} /></div><span className="mono">System state</span><strong>{row.settings.enabled ? "Earning XP" : "Paused"}</strong><p>{row.settings.enabled ? `${row.settings.gain.min}–${row.settings.gain.max} XP every ${row.settings.gain.cooldownSeconds}s` : "Enable XP below when you are ready."}</p></div>
