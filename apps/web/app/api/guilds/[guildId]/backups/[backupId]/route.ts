...
export async function GET(_: Request, context: { params: Promise<{ guildId: string; backupId: string }> }) {
  ...
  return new NextResponse(JSON.stringify(snapshot.payload, null, 2), { headers: { "content-type": "application/json", "content-disposition": `attachment; filename="inochi-${guildId}-${snapshot.createdAt.toISOString().slice(0, 10)}.json"` } });
}
