      ...
      <div className="playground-controls">
        <label>
          ...
          <input
            ...
            max="5"
            step=".1"
            value={settings.curve.cubic}
            style={{ "--range-progress": `${(settings.curve.cubic / 5) * 100}%` } as CSSProperties}
            onChange={(event) => update("cubic", Number(event.target.value))}
          />
          <strong>{settings.curve.cubic}</strong>
        </label>
        <label>
          Quadratic
          <input
            ...
            max="150"
            step="5"
            value={settings.curve.quadratic}
            style={{ "--range-progress": `${(settings.curve.quadratic / 150) * 100}%` } as CSSProperties}
            onChange={(event) => update("quadratic", Number(event.target.value))}
          />
          <strong>{settings.curve.quadratic}</strong>
        </label>
        <label>
          Linear
          <input
            ...
            max="500"
            step="10"
            value={settings.curve.linear}
            style={{ "--range-progress": `${(settings.curve.linear / 500) * 100}%` } as CSSProperties}
            onChange={(event) => update("linear", Number(event.target.value))}
          />
          <strong>{settings.curve.linear}</strong>
        </label>
      </div>
    </div>
