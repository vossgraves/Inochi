...
export function CurvePreview({ settings, compactView = false }: { settings: GuildSettings; compactView?: boolean }) {
  ...
  const maxLevel = Math.max(1, deferred.curve.maxLevel);
  const level = Math.min(selected, maxLevel);
  const sampleCount = Math.min(100, maxLevel);
  const levels = Array.from({ length: sampleCount + 1 }, (_, index) => Math.max(1, Math.round(1 + index * (maxLevel - 1) / sampleCount)));
  const uniqueLevels = [...new Set(levels)];
  const values = uniqueLevels.map((point) => mode === "total" ? xpForLevel(point, deferred) : xpBetweenLevels(Math.max(0, point - 1), deferred));
  const transformed = values.map((value) => scale === "log" ? Math.log10(Math.max(1, value)) : value);
  const maximum = Math.max(1, ...transformed);
  const points = uniqueLevels.map((point, index) => {
  ...
  const benchmarkLevels = [...new Set([1, 5, 10, 25, 50, 100, maxLevel].filter((value) => value <= maxLevel))];
  ...
  return <div className={`curve-card ${compactView ? "curve-card-compact" : ""}`}>
    ...
    <div className="curve-plot" aria-label={`${mode === "total" ? "Total XP" : "XP per level"} curve through level ${maxLevel}`}>
      <svg viewBox="0 0 100 106" role="img" preserveAspectRatio="none">
        ...
        <circle cx={selectedX} cy={selectedY} r="1.8" className="curve-dot" />
      </svg>
    ...
    <div className="curve-selection">
      ...
      <input id={`${id}-level`} type="range" min="1" max={maxLevel} value={level} onChange={(event) => setSelected(Number(event.target.value))} />
    ...
    {!compactView && <>
      <div className="curve-options"><button type="button" className="text-button" onClick={() => setScale((value) => value === "linear" ? "log" : "linear")}>{scale === "linear" ? "Use logarithmic scale" : "Use linear scale"}</button><span className="mono status">L³ {deferred.curve.cubic} · L² {deferred.curve.quadratic} · L {deferred.curve.linear}</span></div>
