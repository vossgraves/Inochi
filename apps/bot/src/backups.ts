...
async function deliverPending(client: Client, guildId: string, settings: GuildSettings) {
  ...
  for (const snapshot of pending) {
    ...
    const embed = new EmbedBuilder().setColor(0x8ba8ff).setTitle("Scheduled Inochi backup").setDescription(`Full backup created for **${(payload.members?.length ?? 0).toLocaleString()} members**.\nChecksum: \`${snapshot.checksum.slice(0, 16)}…\``).setTimestamp(snapshot.createdAt);
